  ...
  <>
    <div className="container mx-auto px-8 md:px-20 flex flex-wrap items-center justify-center lg:justify-between">
      <div className="w-full lg:w-[60%]">
        {data.heading && (
          <h1 className="font-bold sm:text-4xl text-TDarkBlue capitalize pt-20">
            {data.heading}
          </h1>
        )}

        {data.description && (
          <p className="sm:text-2xl text-xs sm:leading-relaxed text-gray-600 my-6">
            {data.description}
          </p>
        )}
      </div>
    ...
    {data.cardsData.map((v, i) => (
      <ServiceDetailCard cardData={v} key={i} />
    ))}
  </>
);
